...
11. **Legacy microdata schema lingering site-wide.** Header logo div carries `itemscope itemtype="http://schema.org/Organization"` with only `itemprop="url"` + `itemprop="logo"` (Impulse default from the pre-JSON-LD era). Comprehensive audit 2026-05-09 across static live HTML and JS-rendered DOM (live tested URL: `/products/beech-bookcase-with-full-side-panels-6-shelves-165x60x25-cm`) confirms it is the *only* non-JSON-LD structured-data signal anywhere on the site: zero RDFa attributes, zero orphan `itemprop` outside `itemscope` blocks, zero other microdata fragments, no third-party widgets (Trustpilot included) injecting any schema dialect post-load. Every other schema source already speaks clean JSON-LD: v1.0 dev ships `Product` + `Brand` + `Offer` + `BreadcrumbList` JSON-LD on PDPs, all valid. The fragment ships from the header partial, so every page emits it (homepage, PDP, collection, blog all verified, JS-rendered DOM confirmed). It is incomplete (no `name`, no `address`, no `sameAs`), it is the older-and-weaker schema dialect Google has discouraged since 2018, and it will collide with the canonical JSON-LD `Organization` block planned in §3 (master doc bullet) - two competing Organization records on the same page degrades entity resolution. Strip the microdata; ship a single source-of-truth JSON-LD Organization block in `theme.liquid` head. After cleanup, every schema signal on the site speaks one dialect (JSON-LD), describes one entity model, and offers one extraction path for AI engines and Google.
